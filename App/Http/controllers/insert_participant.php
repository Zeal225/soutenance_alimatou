...
// $structure_superieur=$_POST['structure_superieur'];
    ...
    try {
        //code...
        $select = $conn->query('SELECT max(ID_PARTICIPANT) as ID_PARTICIPANT from PARTICIPANT');    
        $retour = $select->fetch();
        $id = $retour['ID_PARTICIPANT']+1;

        $conn->beginTransaction();

            $conn->exec("INSERT INTO PARTICIPANT(ID_PARTICIPANT, NOM_PRENOM_PARTICIPANT, FONCTION_PARTICIPANT, STRUCTURE_PARTICIPANT, LOGIN_PARTICIPANT, TEL_PARTICIPANT, EMAIL_PARTICIPANT) VALUES('$id','$nom_prenom','$fonction','$structure','$login','$tel','$mail')");
            var_dump($conn);
        $conn->commit();
